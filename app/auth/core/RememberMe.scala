...
object RememberMe extends CookieBaker[RememberMe] {
  ...
  val DEFAULT_MAX_AGE = 60*60*24*14
  ...
  override val maxAge = Play.maybeApplication.flatMap(_.configuration.getInt("rememberMe.maxAge")).orElse(Option(DEFAULT_MAX_AGE))
